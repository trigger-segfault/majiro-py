...
def main(argv:list=None) -> int:
    if argv is None:
        import sys
        argv = sys.argv[1:]
    enable_vt_mode = Ellipsis
    if len(argv) >= 1:
        if argv[0].lower() in ('true', 'y','yes', '1'): enable_vt_mode = True
        if argv[0].lower() in ('false', 'n','no', '0'): enable_vt_mode = False
        if argv[0].lower() in ('...', 'ellipsis', 'x', '-', ''): enable_vt_mode = Ellipsis
    import_colorama = Ellipsis
    if len(argv) >= 2:
        if argv[1].lower() in ('true', 'y','yes', '1'): import_colorama = True
        if argv[1].lower() in ('false', 'n','no', '0'): import_colorama = False
        if argv[1].lower() in ('...', 'ellipsis', 'x', '-', ''): import_colorama = Ellipsis
    print(f'ansi_support() -> {ansi_support(enable_vt_mode, import_colorama)} [vt_mode={vt_mode_enabled()}] [colorama={colorama_enabled()}]')

    if len(argv) > 2:
        enable_vt_mode = Ellipsis
        if len(argv) >= 3:
            if argv[2].lower() in ('true', 'y','yes', '1'): enable_vt_mode = True
            if argv[2].lower() in ('false', 'n','no', '0'): enable_vt_mode = False
            if argv[2].lower() in ('...', 'ellipsis', 'x', '-', ''): enable_vt_mode = Ellipsis
        import_colorama = Ellipsis
        if len(argv) >= 4:
            if argv[3].lower() in ('true', 'y','yes', '1'): import_colorama = True
            if argv[3].lower() in ('false', 'n','no', '0'): import_colorama = False
            if argv[3].lower() in ('...', 'ellipsis', 'x', '-', ''): import_colorama = Ellipsis
        print(f'ansi_support() -> {ansi_support(enable_vt_mode, import_colorama)} [vt_mode={vt_mode_enabled()}] [colorama={colorama_enabled()}]')
    return 0
