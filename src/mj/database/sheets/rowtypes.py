...
from ...script.flags import MjoType, MjoScope
from ...crypt import hash32

from ...name import GROUP_SYSCALL, GROUP_LOCAL, FUNC_MAIN, joingroup
from ...signature import IdentifierSig, SyscallSig, FunctionSig, VariableSig, LocalSig
...
class _ColBase(abc.ABC):
    ...
    def __init__(self, col_name:str, python_name:str=...):
        ...
        self.python_name = col_name.lower() if python_name is Ellipsis else python_name
    @abc.abstractmethod
...
class _RowBase:
    ...
    def to_row(self, *, preserve:bool=False) -> OrderedDict:
        ...
        return odict
    def verify(self) -> bool: return True # default implementation, no verification
...
class _RowHashBase(_RowBase, cols=()): #TODO: better way to handle base class?
    ...
    @property
    def identifier(self) -> IdentifierSig: raise NotImplementedError(f'{self.__class__.__name__}.identifier')
    @property
    def fullname(self) -> str: raise NotImplementedError(f'{self.__class__.__name__}.fullname')
    @property
    def confirmed(self) -> bool: return self.status in (Status.UNHASHED, Status.COLLISION, Status.CONFIRMED)
    @property
    def unhashed(self) -> bool: return self.status in (Status.UNHASHED, Status.CONFIRMED)  # no collisions
    def verify(self) -> bool: return not self.confirmed or (hash32(self.fullname) == self.hash)
...
class RowSyscall(_RowHashBase,cols=(ColHash('Hash'), ColAddress('Address','source'), ColType('Return','type'),
    ...
    def __init__(self, *args, **kwargs):
        ...
        super().__init__(*args, **kwargs)
    @property
    def scope(self) -> MjoScope: return MjoScope.FUNCTION
    @property
    def group(self) -> str: return GROUP_SYSCALL
    @property
    def identifier(self) -> SyscallSig:
        return SyscallSig(self.name or '', is_void=self.type is Typedef.VOID, doc=self.notes)
    @property
    def fullname(self) -> str: return joingroup(self.name or '', self.group)
...
class RowVariable(_RowHashBase,cols=(ColHash('Hash'), ColText('Source'), ColScope('Scope'), ColType('Type'),
    ...
    def __init__(self, *args, **kwargs):
        ...
        super().__init__(*args, **kwargs)
    # @property
    # def group(self) -> str: return GROUP_LOCAL
    @property
    def identifier(self) -> VariableSig: return VariableSig(self.name or '', group=self.group or '', doc=self.notes)
    @property
    def fullname(self) -> str: return joingroup(self.name or '', self.group)
...
class RowLocal(_RowHashBase,cols=(ColHash('Hash'), ColType('Type'), ColText('Name'),
    ...
    def group(self) -> str: return GROUP_LOCAL
    @property
    def identifier(self) -> LocalSig: return LocalSig(self.name or '', doc=self.notes)
    @property
    def fullname(self) -> str: return joingroup(self.name or '', self.group)
...
class RowGroup(_RowHashBase,cols=(ColHash('Hash'), ColText('Source'), ColText('Name'),#, 'group'),
    ...
    def scope(self) -> MjoScope: return MjoScope.FUNCTION
    @property
    def group(self) -> Optional[str]: return self.name
    @property
    def identifier(self) -> FunctionSig: return FunctionSig(FUNC_MAIN, group=self.group or '', doc=self.notes)
    @property
    def fullname(self) -> str: return joingroup(FUNC_MAIN, self.group) #f'$main@{self.group or ""}'
...
class RowCallback(_RowHashBase,cols=(ColHash('Hash'), ColText('Invoked by','source'),
    ...
    def __init__(self, *args, **kwargs):
        ...
        super().__init__(*args, **kwargs)
    @property
    def scope(self) -> MjoScope: return MjoScope.UNKNOWN
    @property
    def identifier(self) -> IdentifierSig: return IdentifierSig(self.name or '', doc=self.notes)
    @property
    def fullname(self) -> str: return self.name or ''
...
class RowGame(_RowBase,cols=(ColDate('Release'), ColText('Developer'), ColText('Name'),
    ...
    name:Optional[str]
    build:Optional[datetime]
    notes:str

    def __init__(self, *args, **kwargs):
        self.release = None #_DATE_HINT_
        self.developer = None #''
        self.name = None #''
        self.build = None #_DATE_HINT_
        self.notes = ''
        super().__init__(*args, **kwargs)
...
del chain, abc, enum, Any, Dict, List, Optional, Union  # cleanup declaration-only imports
