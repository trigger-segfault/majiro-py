...
_POPARRAY_TRANSITIONS   = (("i[i#d].i", "i[i#d]."), ("n[i#d].f", "n[i#d]."), ("s[i#d].s", "s[i#d]."), (), (), ())
...
def define_opcode(value:int, mnemonic:str, op:str, encoding:str, transition:str, *aliases:str) -> None:
    ...
    if existing is not None:
        raise ValueError(f'Opcode {opcode.mnemonic!r} value 0x{opcode.value:03x} already defined by {existing.mnemonic!r}')
    ...
    for alias in ([mnemonic] + list(aliases)): 
        ...
        if existing is not None:
            raise ValueError(f'Opcode {opcode.mnemonic!r} 0x{opcode.value:03x} alias {alias!r} already defined by {existing.mnemonic!r} 0x{existing.value:03x}')
...
define_opcode(0x830, "br.case", None, "j", "p.", "br.v", "jmp.v")    # !!non-final name!!
define_opcode(0x831, "bne.case", None, "j", "p.", "bne.v", "jne.v")  # !!non-final name!!
define_opcode(0x832, "bge.case", None, "j", "p.", "bge.v", "jge.v")  # !!non-final name!!
define_opcode(0x833, "ble.case", None, "j", "p.", "ble.v", "jle.v")  # !!non-final name!!
define_opcode(0x838, "blt.case", None, "j", "p.", "blt.v", "jlt.v")  # !!non-final name!!
define_opcode(0x839, "bgt.case", None, "j", "p.", "bgt.v", "jgt.v")  # !!non-final name!!

define_opcode(0x834, "syscall",  None, "ha", "[*#a].*")
define_opcode(0x835, "syscallp", None, "ha", "[*#a].")

define_opcode(0x836, "argcheck", None, "t", ".[#t]", "argchk", "sigchk")  # function arguments signature
