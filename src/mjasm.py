...
def print_script(filename:str, script:MjoScript, *, options:ILFormat=ILFormat.DEFAULT):
    ...
    disassembler.print_readmark(script, options=options)
    # print()

    for function in cfg.functions:
        print()
        disassembler.print_function(function, options=options)
        for i,basic_block in enumerate(function.basic_blocks):
            print(' ', end='')
            disassembler.print_basic_block(basic_block, options=options)
            for instruction in basic_block.instructions:
                reskey = disassembler.get_resource_key(script, instruction, options=options)
                print('  ', end='')
                disassembler.print_instruction(instruction, options=options, resource_key=reskey)
            if i + 1 < len(function.basic_blocks):
                print(' ')
        disassembler.print_function_close(function, options=options)
        # print()
...
""")
    ...
    try:  # try adding research module
        ...
        def _init_args(args): pass
        def do_research(*args, **kwargs): pass
     ...
     for infile,outfile in (args.disasm or []):
        ...
        if os.path.isdir(infile):  # directory of .mjo files
            ...
            for name in os.listdir(infile):
                ...
                if not os.path.isfile(path) or not os.path.splitext(path)[1].lower() == '.mjo':
                    continue
                print('Disassembling:', name) #.ljust(len(last_name)*2))  #HACK: *2 to handle double-width CJK
                ...
                script = read_script(path)
                ##disassembler.disassemble_script(path, script, outpath, options=options)
            ...
            if outfile is None:
                ...
                outpath = os.path.join(outfile, os.path.splitext(name)[0] + '.mjil')
            script = read_script(infile)
            ##disassembler.disassemble_script(infile, script, outpath, options=options)
