...
from mjotool._util import DummyColors, Colors
from mjotool.flags import MjoType
from mjotool.crypt import hash32
from mjotool.script import KNOWN_SYSCALLS, SPECIAL_VARIABLES
# from mjotool.script import MjoScript
...
class MjsReader:
    def __init__(self, filename:str, *, encoding:str='cp932'):
    ...
    def parse_preprocessor_if(self, pre_token:ParseToken):
        ...
        if name in ('#if', '#elif'):
            ...
            print('#else // {}'.format(int(not self.pre_ignore)))
        elif name == '#endif':
            if not self.pre_ifstack:
                raise Exception('unexpected preprocessor {} without #if on line {}'.format(name, self.line_number))
    ...
    def parse_preprocessor(self, pre_token:ParseToken):
        ...
        if name == '#define':
            ...
            print('#define {} {}'.format(define_name, define_value))
        elif name == '#include':
            token = self.parse_token_skipws()
            ...
            # print('#include {}'.format(token.text))
        elif name == '#group':
            token = self.parse_token_skipws()
    ...
    def parse_funcsig(self, func_token:ParseToken):
        has_return:bool = func_token.text == 'func'
        token = self.parse_token_skipws()
        if token.type is not TokenType.IDENTIFIER:
            raise Exception('unexpected token {!r} after {} function declaration on line {}'.format(token.text, token.text, self.line_number))
        function = FunctionSig(self.defines.get(token.text, token.text), has_return=has_return, group=self.current_group)
...
def load_mjs_hashes(filename:str, var_hashes:Dict[int,str], func_hashes:Dict[int,str], group_names:Set[str]):
...
def main(argv:list=None) -> int:
    ...
    if not args.inputs + args.hashes + args.syscalls + args.groups + args.all:
        load_mjs_hashes('../data/mjs/adv.mjh.bak', var_hashes, func_hashes, group_names)

    ## LOAD FILES ##

    for scriptfile in args.inputs:
        load_mjs_hashes(scriptfile, var_hashes, func_hashes, group_names)
