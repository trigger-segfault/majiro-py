#!/usr/bin/env python3
#-*- coding: utf-8 -*-
"""Quick utility to try and find any known hashes matching the specified input
"""
...
def main(argv:list=None) -> int:
    ...
    parser.add_argument('name', metavar='NAME',
        help='name to try hashing')
    parser.add_argument('-c', '--constant', dest='format', default='syscall', const='constant', action='store_const', required=False,
        help='input treated as absolute (no prefix and no postfix)')
    parser.add_argument('-s', '--syscall', dest='format', const='syscall', action='store_const', required=False,
        help='input treated as syscall (\"$\" prefix and \"@MAJIRO_INTER\" postfix) [default]')
    parser.add_argument('-f', '--function', dest='format', const='function', action='store_const', required=False,
        help='input treated as function (\"$\" prefix and no postfix)')
    parser.add_argument('-l', '--local', dest='format', const='local', action='store_const', required=False,
        help='input treated as local var (\"_\" prefix and \"@\" postfix)')
    ...
    FORMATS:dict = {
        'constant':  '{0}',
        'syscall':  '${0}@MAJIRO_INTER',
        'function': '${0}',
        'local':    '_{0}@',
    }
    nformat:str = FORMATS[getattr(args, 'format', 'syscall')]
    fullname:str = nformat.format(args.name)
    fullhash:int = crc32(fullname.encode('cp932'))

    ###########################################################################

    print(f'crc32({fullname!r}) -> 0x{fullhash:08x}')
    results = [f'|{(i+2):<3d}| 0x{h:08x} | {fullname}' for i,h in enumerate(SYSCALL_LIST) if h == fullhash]
